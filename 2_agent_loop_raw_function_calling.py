...
def get_product_price(product: str) -> float:
    """Look up the prive of a product in the catalog."""
    print(f"🛠️ Executing get_product_price tool for {product}")
    prices = {
        "laptop": 1299.99,
        "headphones": 149.95,
        "keyboard": 89.50,
...
def apply_discount(price: float, discount_tier: str) -> float:
    """Apply discount based on tier and return final price.
    Avaliable tiers: " bronze, silver, gold."""
    print(f"🛠️ Executing apply_discount tool for {price} with tier {discount_tier}")
    discount_percentages = {"bronze": 5, "silver": 12, "gold": 23}
    discount = discount_percentages.get(discount_tier, 0)
    return round(price * (1 - discount / 100), 2)

# Difference 2 : Without @tool, we must MANUALLY  define the JSON schema for each function.
# This is exactly what LangChain @tool decorator generates automatically
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        if not tool_calls:
            print("AI Final Response:", ai_message.content)
        ...
        if not tool_to_use:
            print(f"❌ Error: No tool found with name {tool_name}")
